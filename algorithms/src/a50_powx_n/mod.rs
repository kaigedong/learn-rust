...
impl Solution {
    ...
    pub fn my_pow(x: f64, n: i32) -> f64 {
        ...
        if n < 0 {
            1.0 / Self::my_pow_helper(x, n.abs(), &mut seen_pow)
        } else {
            Self::my_pow_helper(x, n, &mut seen_pow)
        }
    }
    ...
    // 当n % 2 == 1:
    // x^n = x^(n/2) * x^(n/2) * x
    fn my_pow_helper(x: f64, n: i32, seen_pow: &mut HashMap<i32, f64>) -> f64 {
        // 存储已经在HashMap中存储的x的多少次方的值
        if seen_pow.contains_key(&n) {
            *seen_pow.get(&n).unwrap()
        } else {
            let out = if n % 2 == 0 {
                Self::my_pow_helper(x, n / 2, seen_pow) * Self::my_pow_helper(x, n / 2, seen_pow)
            } else {
                Self::my_pow_helper(x, n / 2, seen_pow)
                    * Self::my_pow_helper(x, n / 2, seen_pow)
                    * Self::my_pow_helper(x, 1, seen_pow)
            };
